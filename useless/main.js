...
async function submit() {
  ...
  var { data, error } = await supabase.from('users').select('id');
  if (error) {
    trans('<b>An error occured.</b>');
    document.querySelector('#ed').innerText = 'An error related to the database occured. Try again?';
  } else if (data.map(v => { return v.id }).includes(document.querySelector('#uin').value)) {
    trans('<b>That user is already in the database!</b>');
    document.querySelector('#ed').innerText = 'That user is already in the database. Try someone else?';
  } else {
    document.querySelector('#prog').value = 1;
    fetch(`https://api.allorigins.win/raw?url=${encodeURIComponent('https://community.gethopscotch.com/api/v2/users/'+document.querySelector('#uin').value)}`)
      .then(response => {
      ...
      .then(async data => {
        if(Object.entries(data).length == 0) {
          trans('<b>That user does not exist?</b>');
          document.querySelector('#ed').innerText = 'That user does not exist :( — try someone else?';
          return;
        }
        document.querySelector('#prog').value = 2;
        trans('One last thing...')
        var { error } = await supabase.from('users').insert([
          { id: document.querySelector('#uin').value, val: '{}' }
        ]);
        if (error) {
          trans('<b>An error occured.</b>');
          document.querySelector('#ed').innerText = 'An error related to the database occured. Try again?';
          ...
          trans('<b>Yay!</b>');
          document.querySelector('#ed').innerText = 'That user has been added to the database. All you need to do is wait!';
        }
      }).catch(e => {
        trans('<b>Whoooooooops.</b>');
        document.querySelector('#ed').innerText = 'An error related to fetch occured. Try another project?';
      })
  }
...
async function data() {
  ...
  var { data, error } = await supabase.from('users').select('id');
  if (error) {
    document.querySelector('#data').innerText = 'An error related to the database occured.';
  } else if (!data.map(v => { return v.id }).includes(document.querySelector('#uit').value)) {
    document.querySelector('#data').innerText = 'That user is not in the database (yet?).';
  } else {
    var val = await supabase.from('users').select('*').match({ 'id': document.querySelector('#uit').value });
    if (val.data[0].val == '{}') {
      document.querySelector('#data').innerText = 'No data available (yet!)';
    } else {
      val = val.data[0];
      val.things = JSON.parse(val.val);
